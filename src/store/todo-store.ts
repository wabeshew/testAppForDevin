...
import { type TodoId, type TodoText, createTodoId, createTodoText } from '../types/branded-types';
...
export const addTodoAtom = atom(null, (get, set, params: string | AddTodoParams) => {
  try {
...
export const updateTodoTextAtom = atom(null, (get, set, id: TodoId, text: string) => {
  try {
...
export const updateTodoDeadlineAtom = atom(
  ...
  (get, set, id: TodoId, deadline: Date | undefined) => {
    ...
    set(
      todosAtom,
      todos.map((todo) => (todo.id === id ? { ...todo, deadline } : todo))
    );
    return true;
  }
);

export const updateTodoPriorityAtom = atom(null, (get, set, id: TodoId, priority: Priority) => {
  const todos = get(todosAtom);
  set(
    todosAtom,
...
export const setFilterAtom = atom(null, (_, set, filter: FilterType) => {
